export default function Timeline() {
  ...
  return ( 
    <div>
      <div className="grid place-content-center gap-y-10 relative after:absolute after:w-2 after:h-[calc(100%+40px)] after:bg-zinc-900 after:left-1/2 after:-z-50 after:rounded-full after:-translate-y-5">
        {timeline.map((element, index) => element &&
          <div className={`text-sm sm:text-base text-zinc-100 w-full sm:w-64 md:w-96 p-6 relative bg-zinc-900 rounded-md shadow shadow-zinc-950 sm:after:absolute sm:after:w-6 sm:after:h-6 sm:after:bg-zinc-900 sm:after:rounded-full sm:after:top-1/2 sm:after:outline sm:after:-outline-offset-2 sm:after:outline-zinc-300 ${index % 2 === 0 ? 'sm:mr-3 sm:-translate-x-1/2 sm:after:-translate-y-1/2 sm:after:translate-x-1/2 sm:after:right-0' : 'sm:-mr-2 sm:ml-auto sm:translate-x-1/2 sm:after:-translate-y-1/2 sm:after:-translate-x-1/2 sm:after:left-0'}`} key={element.id}>
            <p className="font-semibold">{element.date}</p>
            <p>{element.description}</p>
          </div>  
        )}
        <svg className="w-12 h-12 fill-zinc-900 absolute top-[calc(100%-5px)] left-1/2 -translate-x-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z"/></svg>
      </div>
